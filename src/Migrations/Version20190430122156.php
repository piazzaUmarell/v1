 ...
 */
final class Version20190430122156 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__episode AS SELECT id, series_id, title, number, abstract, description, publication_date, source, slug FROM episode');
        $this->addSql('DROP TABLE episode');
        $this->addSql('CREATE TABLE episode (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, series_id INTEGER DEFAULT NULL, title VARCHAR(255) NOT NULL COLLATE BINARY, number NUMERIC(10, 2) DEFAULT NULL, abstract CLOB NOT NULL COLLATE BINARY, description CLOB NOT NULL COLLATE BINARY, publication_date DATETIME DEFAULT NULL, source VARCHAR(255) NOT NULL COLLATE BINARY, slug VARCHAR(255) NOT NULL COLLATE BINARY, duration VARCHAR(255) NOT NULL, CONSTRAINT FK_DDAA1CDA5278319C FOREIGN KEY (series_id) REFERENCES series (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO episode (id, series_id, title, number, abstract, description, publication_date, source, slug) SELECT id, series_id, title, number, abstract, description, publication_date, source, slug FROM __temp__episode');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('DROP INDEX IDX_DDAA1CDA5278319C');
        $this->addSql('DROP INDEX slug_idx');
        $this->addSql('CREATE TEMPORARY TABLE __temp__episode AS SELECT id, series_id, title, slug, number, abstract, description, publication_date, source FROM episode');
        $this->addSql('DROP TABLE episode');
        ...
        $this->addSql('INSERT INTO episode (id, series_id, title, slug, number, abstract, description, publication_date, source) SELECT id, series_id, title, slug, number, abstract, description, publication_date, source FROM __temp__episode');
        $this->addSql('DROP TABLE __temp__episode');
        $this->addSql('CREATE INDEX IDX_DDAA1CDA5278319C ON episode (series_id)');
        $this->addSql('CREATE UNIQUE INDEX slug_idx ON episode (slug)');
    }
