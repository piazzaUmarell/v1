<template>
    <header class="filter lg:flex px-8 lg:px-32" :class="{sticky: scrolledPast}">
        <router-link :to="homepageRoute" class="hover:no-underline no-underline">
            <h1 class="lg:mx-8 text-center md:ml-4 md:text-left">Piazza Umarell</h1>
        </router-link>
        <div class="input-container">
            <label class="ml-2" for="filter">
...
<style lang="scss" scoped>
    ...
    .filter {
        ...
        @apply text-lg mb-16 bg-white py-3 w-full items-baseline;

        &.sticky {
            @apply sticky pin-t pl-4;
        ...
        h1 {
            ...
            @apply py-2 text-white rounded-full text-3xl;
        }

        .input-container {
            @apply rounded-full border border-grey-lighter bg-grey-lighter px-4 outline-none pt-2 pb-1 flex items-baseline flex-1;
